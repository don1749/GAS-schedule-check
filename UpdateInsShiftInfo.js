...
var curMonth = ss4.getRange('B6').getValue();
var dayNum = ss4.getRange('B7').getValue();

function updateShiftInfo() {
  var ss1 = spread[0]; //データ一覧のシートへアクセス
  var curr = ss4.getRange('B2').getValue();
  for (var i = curr; i < ins_num; i++)
  {
   var ins_name = ss1.getRange(2,i+3).getValue() + " 先生";//注）先生名のフォーマット：〇〇 先生（スペースは半角）
    ...
    var requestedList = new Array();//この先生を希望した生徒リス
    for (var j = 0; j < stu_num; j++) 
    {
      var stuRequest = ss3.getRange(6,j+2).getValue();
